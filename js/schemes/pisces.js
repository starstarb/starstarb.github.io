...
var Affix = {
  init: function (element, options) {
    ...
    window.addEventListener('click', this.checkPositionWithEventLoop.bind(
      this));
  ...
  getState: function (scrollHeight, height, offsetTop, offsetBottom) {
    ...
    if (this.affixed === 'bottom') {
      if (offsetTop != null) return this.unpin <= this.element
        .getBoundingClientRect().top ? false : 'bottom';
      return scrollTop + targetHeight <= scrollHeight - offsetBottom ?
        false : 'bottom';
    }
    let initializing = this.affixed === null;
    let colliderTop = initializing ? scrollTop : this.element
  ...
  checkPosition: function () {
    ...
    let affix = this.getState(scrollHeight, height, offsetTop,
    offsetBottom);
...
window.addEventListener('DOMContentLoaded', () => {
  Affix.init(document.querySelector('.sidebar-inner'), NexT.utils
    .getAffixParam());
});
